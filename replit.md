...
- **Fixed sort functionality not working** - Resolved bug where sort filters (by added, rating, A-Z, etc.) were not working due to a condition that prevented sorting in the "all" category. Removed the restrictive `category.category_id!='all'` check so sorting now works properly in all categories including the combined "all" view.
- **Restored NEW badge for recently added content** - Re-implemented the NEW badge that appears on the first 10 movies/series when sorted by "added". The badge was accidentally removed during the code restoration but is now back with proper logic checking the current sort order and item position.
- **Added fallback background when API is unavailable** - Implemented automatic fallback to local background1.png image when the theme API is not working or themes array is empty. This prevents the app from showing an empty/blank background and ensures a proper visual experience even when offline or during API failures.
