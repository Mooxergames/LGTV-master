...
- **Fixed live TV preview vs fullscreen timing issue** - Resolved a critical timing problem with video display area calculations when switching from channel preview to fullscreen mode. The fix addresses Samsung webapis.avplay.setDisplayRect() timing issues by ensuring setDisplayArea() is called after CSS positioning changes are fully applied, with conditional logic for preview vs fullscreen modes and proper handling of initial fullscreen entry paths.
