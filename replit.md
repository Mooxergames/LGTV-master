...
- **Implemented 4K/8K UHD streaming optimization with DPR scaling** - Added automatic 4K/8K detection and Samsung AVPlay configuration for both live TV channels and VOD/Series content. When content is detected as 4K or 8K (from channel/movie name), the player automatically configures Samsung's SET_MODE_4K streaming property and ADAPTIVE_INFO with appropriate max resolution (3840x2160 for 4K, 7680x4320 for 8K). Implemented devicePixelRatio scaling for setDisplayRect to use physical pixels (3840x2160) instead of CSS pixels (1920x1080) on 4K TVs, ensuring native resolution playback without quality-degrading upscaling. Detection works for content containing keywords: 4K, UHD, 2160P, 8K in the title.
- **Added TV 4K capability detection** - Implemented TV hardware 4K support detection using Tizen SystemInfo API (tizen.systeminfo.getPropertyValue('DISPLAY')) to check actual screen resolution. Runs once at app initialization and logs TV capabilities without requiring special Samsung ProductInfo privileges.
