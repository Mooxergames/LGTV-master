...
- **Fixed preview screen and zoom functionality in live TV** - Restored critical media_player.setDisplayArea() call in channel_operation.js zoomInOut() function that was accidentally removed in previous commits. This fix resolves issues with toggling between fullscreen and preview modes, ensuring proper video display area adjustment after CSS changes. The function now correctly checks the full_screen_video flag and calls setDisplayArea() with a 100ms delay to allow CSS transitions to complete.
- **Added NEW badge to recently added content** - Implemented red "NEW" indicator badge on top-left corner of movie and series posters for the last 10 added items when sorted by date added. Features responsive CSS styling, smart display logic based on sort order, and proper positioning that doesn't conflict with favorite star badges.
- **Repositioned Subtitle Settings in settings menu** - Moved subtitle settings menu item higher in the settings menu, positioned immediately after the theme option for easier access and better user experience.
- **Implemented Samsung 4K/8K UHD video support** - Added comprehensive UHD capability detection using Samsung ProductInfo API (isUdPanelSupported() and is8KPanelSupported()) according to official Samsung documentation. Features include TV capability checking during player initialization, resolution validation with visual indicators (✓ for supported, → for downscaled), detailed UHD debugging logs, and proper fallback handling for non-UHD TVs. App now properly detects and displays whether 4K (3840x2160) and 8K (7680x4320) content can be played natively or requires downscaling.
- **Fixed Samsung InvalidStateError in live TV** - Resolved critical Samsung webapis.avplay InvalidStateError by removing overlapping setDisplayArea() calls during video initialization. The fix eliminates the immediate setDisplayArea() call after webapis.avplay.open() and now only calls it once after buffering completes, preventing rapid-fire setDisplayRect attempts that put Samsung's player into invalid states. Added proper timer cleanup to prevent late calls after player teardown.
