...
- **Fixed VOD video resuming when switching to Live TV** - Enhanced LG player cleanup in stop() and close() methods by properly removing video source and resetting video element state. Previously, switching from VOD to Live TV would cause the old movie to keep trying to resume playback. Now includes removeAttribute('src') and load() calls to completely clean up the video element, preventing "play() interrupted by pause()" errors and ensuring smooth transitions between different content types.
