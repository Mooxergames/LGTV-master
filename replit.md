...
- **Fixed home page preview interfering with Live TV** - Added route guards (current_route === 'home-page' checks) to all media_player.playAsync() calls in home_operation.js to prevent home page preview from playing when user navigates to Live TV. Stops slick autoplay slider and resets preview state variables when exiting home page. This resolves the issue where VOD movies would play in Live TV preview screen instead of the first live channel.
- **Fixed live TV preview screen and zoom functionality** - Restored critical media_player.setDisplayArea() call in channel_operation.js zoomInOut() function with correct logic flow (checking if full_screen_video is TRUE for fullscreen, FALSE for preview). Added proper timer cleanup in player.js stop() and close() methods to prevent late setDisplayArea() calls after player teardown. These fixes resolve toggling issues between fullscreen and preview modes, eliminate "video on left side with black space" display problems, and improve overall player stability on Samsung Tizen devices.
- **Restored Samsung live TV player to original working state** - Reverted player.js and channel_operation.js to commit 5b4f1f7 (stable version before aspect ratio and fullscreen modifications). The original simple implementation calls setDisplayArea() immediately after webapis.avplay.open() without delays or conditional logic, ensuring smooth preview/fullscreen transitions and proper channel switching on Samsung Tizen devices.
