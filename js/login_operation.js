...
var login_page = {
    ...
    startApp: function (data) {
        ...
        themes.map(function (theme, index) {
            htmlContents +=
                '<div class="modal-operation-menu-type-3 theme-modal-option"\
                    onclick="home_page.pickTheme(' +
                index +
                ')"\
                                                    onmouseenter="home_page.hoverThemeModal(' +
                index +
                ')"\
                                                >' +
                theme.name +
        ...
        var bg_img_index = 0;
        var bg_img_index_temp = localStorage.getItem(
            storage_id + "bg_img_index",
        );
        ...
        if (data.pin_code) parent_account_password = data.pin_code;
        ...
        if (!data.mac_registered) {
            ...
            if (data.expire_date < today) {
                ...
                if (data.is_trial == 1) {
                    // will show tiral end message
                    $("#login-play-list-information")
                        .html(
                            "Your trial period will expire on " +
                                data.expire_date +
                                ', please activate your device from <a class="login-page-link">https://flixapp.net/activation</a> <br> <img style="min-width: 400px; max-width: 600px;" src="https://flixapp.net/images/activation-qr-code.svg"> <br> <strong>Scan here to visit the activation page</strong>',
                        )
                        .show();
                    that.login();
                } else {
                    // if expire date less than 7 days later, will show expire date end soon message
                    var date1 = moment().add(7, "days").format("Y-MM-DD");
                    if (data.expire_date < date1)
                        $("#login-play-list-information")
                            .html(
                                "Your account valid date will be ended on " +
                                    data.expire_date +
                                    ', please try extend expire date from <a class="login-page-link">https://flixapp.net/activation</a> <br> <img style="min-width: 400px; max-width: 600px;" src="https://flixapp.net/images/activation-qr-code.svg"> <br> <strong>Scan here to visit the activation page</strong>',
                            )
                            .show();
                    that.login();
                }
    ...
    showNetworkError: function () {
        this.hideLoadImage();
                    ...
                    var playlistCount = playlist_urls
                        ? playlist_urls.length
                        : 0;
                    var choosePlaylistBtn = $(".network-issue-btn").filter(
                        function () {
                            return (
                                $(this).attr("onclick") ===
                                "login_page.showPlaylistSelectionModal()"
                            );
                        },
                    );
    ...
    getSamsungMacAddress: function () {
        ...
        if (typeof tizen === "undefined" || !tizen.systeminfo) {
            console.log(
                "Samsung: Not in Tizen environment (browser/web), using hardcoded MAC",
            );
        ...
        try {
            tizen.systeminfo.getPropertyValue(
                ...
                function (data) {
                    if (
                        data !== undefined &&
                        typeof data.macAddress !== "undefined" &&
                        data.macAddress
                    ) {
    ...
    getSamsungTizenIdMac: function () {
        ...
        try {
            tizen.systeminfo.getPropertyValue(
                ...
                function (error) {
                    console.log(
                        "Samsung: Tizen ID failed, using hardcoded MAC",
                    );
                    that.getSamsungHardcodedMac();
                },
            );
            ...
            webOS.service.request("luna://com.webos.service.sm", {
                ...
                onSuccess: function (inResponse) {
                    if (
                        inResponse &&
                        inResponse.idList &&
                        inResponse.idList.length > 0 &&
                        inResponse.idList[0].idValue
                    ) {
                        console.log("LG: Using LGUDID for MAC address");
                        mac_address = "";
                        var temp = inResponse.idList[0].idValue.replace(
                            /['-]+/g,
                            "",
                        );
                        for (var i = 0; i <= 5; i++) {
    ...
    login: function () {
        ...
        if (mac_valid) {
            ...
            settings.saveSettings(
                "playlist_url",
                playlist_urls[keys.playlist_selection],
                "",
            );
            settings.saveSettings(
                "playlist_url_index",
                keys.playlist_selection,
                "",
            );
            parseM3uUrl();
            this.proceed_login();
    ...
    fallbackToLocalDemo: function () {
        ...
        setTimeout(function () {
            showToast(
                "Demo Mode",
                "You are now using demo content with limited functionality",
            );
        }, 2000);
    },
    ...
    goHomePageWithPlaylistError: function () {
        ...
        // this.hideLoadImage();
        this.goToHomePage();
        if (platform === "samsung") home_page.hoverToMainMenu(6);
        else home_page.hoverToMainMenu(5);
    ...
    proceed_login: function () {
        ...
        if (!mac_valid) {
            console.log(
                "Account expired - blocking proceed_login to prevent network errors",
            );
            this.hideLoadImage();
            return;
        }
    ...
    showNetworkIssueWithPlaylistOptions: function () {
        ...
        if (playlistCount > 1) {
            ...
            choosePlaylistBtn.show();
            console.log(
                "Showing Choose Playlist button - multiple playlists available",
            );
        } else {
            // Hide Choose Playlist button when only one or no playlist exists
            choosePlaylistBtn.hide();
            console.log(
                "Hiding Choose Playlist button - only one or no playlist available",
            );
        }
    ...
    showPlaylistSelectionModal: function () {
        ...
        if (playlist_urls && playlist_urls.length > 1) {
            ...
            for (var i = 0; i < playlist_urls.length; i++) {
                var isSelected =
                    i === keys.playlist_selection ? " playlist-selected" : "";
                playlistOptionsHtml +=
                    ...
                    i +
                    '" ' +
                    'onclick="login_page.selectPlaylistFromError(' +
                    i +
                    ')" ' +
                    'onmouseenter="login_page.hoverPlaylistModalItem(' +
                    i +
                    ')">' +
                    "Playlist " +
                    (i + 1) +
            ...
            $(
                '.playlist-modal-item[data-playlist-index="' +
                    keys.playlist_selection +
                    '"]',
            ).addClass("active");
        }
    ...
    hoverPlaylistModalItem: function (index) {
        ...
        $(".playlist-modal-item").removeClass("active");
        $('.playlist-modal-item[data-playlist-index="' + index + '"]').addClass(
            "active",
        );
    },
    handleMenuClick: function () {
        ...
        if (keys.focused_part === "playlist_selection") {
            if (mac_valid) {
                ...
                showToast(
                    "Sorry",
                    "You can not use our service now, please activate your device at https://flixapp.net/activation and restart your app",
                );
            }
            ...
            if (keys.turn_off_modal == 0) {
                ...
                $("#turn-off-modal").modal("hide");
                var network_display = $("#network-issue-container").css(
                    "display",
                );
                if (network_display !== "block")
                    this.keys.focused_part = "playlist_selection";
                else this.keys.focused_part = "network_issue_btn";
            }
            ...
            var selectedItem = $(".playlist-modal-item")[
                keys.playlist_modal_selection
            ];
            if (selectedItem) {
    ...
    handleMenuUpDown: function (increment) {
        ...
        if (keys.focused_part === "playlist_selection") {
            ...
            moveScrollPosition(
                ...
                this.playlist_doms[keys.playlist_selection],
                "vertical",
                false,
            );
        } else if (keys.focused_part === "turn_off_modal") {
            ...
            if (keys.turn_off_modal < 0) keys.turn_off_modal = 1;
            if (keys.turn_off_modal > 1) keys.turn_off_modal = 0;
            ...
            if (
                this.network_btn_doms.length > 0 &&
                keys.network_issue_btn < this.network_btn_doms.length
            ) {
                $(this.network_btn_doms[keys.network_issue_btn]).addClass(
                    "active",
                );
            }
        } else if (keys.focused_part === "playlist_modal") {
    ...
    handleMenuLeftRight: function (increment) {
        ...
        if (keys.focused_part === "network_issue_btn") {
            ...
            keys.network_issue_btn += increment;
            if (keys.network_issue_btn < 0) keys.network_issue_btn = 0;
            ...
            case tvKey.RETURN:
                if (this.keys.focused_part === "playlist_modal") {
                    ...
                    if (this.network_btn_doms[this.keys.network_issue_btn]) {
                        $(
                            this.network_btn_doms[this.keys.network_issue_btn],
                        ).addClass("active");
                    }
